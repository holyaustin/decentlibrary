...
export default function CreateItem() {
  ...
  async function createSale(url) {
    ...
    let contract = new ethers.Contract(nftvaddress, NFTV.abi, signer)
    ...
    contract = new ethers.Contract(nftvideoaddress, Video.abi, signer)
    let listingPrice = await contract.getListingPrice()
    listingPrice = listingPrice.toString()

    transaction = await contract.createMarketItem(nftvaddress, tokenId, price, { value: listingPrice })
  ...
  return (
    ...
    <div className="flex justify-center">
      <div className="w-1/2 flex flex-col pb-12">
        ...
        />
        <input
          placeholder="Asset Price in METIS (Add 0 for free videos)"
          className="mt-2 border rounded p-4"
        ...
        <input
          type="file"
          name="Asset"
          className="my-1 mt-2 border rounded p-5"
          onChange={onChange}
        />
        <br />
        {
        ...
        <button onClick={createMarket} className="font-bold mt-4 relative rounded-lg bg-blue-600 text-white rounded p-4 shadow-lg">
          Upload Video (mp4)
        </button>
      </div>
    </div>

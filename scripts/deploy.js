...
async function main() {
  ...
  const NFTV = await hre.ethers.getContractFactory("NFTV");
  const nftv = await NFTV.deploy(nftvideo.address);
  await nftv.deployed();
  console.log("nftv deployed to:", nftv.address);

  let config = `
  export const nftlibraryaddress = "${nftlibrary.address}"
  export const nftaddress = "${nft.address}"
  export const nftvideoaddress = "${nftvideo.address}"
  export const nftvaddress = "${nftv.address}"
  `
